...
from src.view_generation.leading_type_views_db import compute_indices_by_leading_type_parallel_db, \
    compute_indices_by_leading_type_db
...
def main(args):
    if args.dataset == "bpi17":
        ...
        compute_views_for_order_management(k=5, selection_method=args.selection_method)
    else:
...
def compute_views(filename, object_types, short_name, file_type="json", params=None, k=2, weight=0.5, selection_method="mmr"):
...
def compute_views_for_bpi17(k=4, weight=0.5,selection_method="mmr"):
    ...
    compute_views(filename, object_types, "BPI17", k=k, weight=weight, selection_method=selection_method)


def compute_views_for_bpi17_csv(k=4, weight=0.5, selection_method="mmr"):
    ...
    assert k <= len(object_types), "k must be less than the number of object types"
    compute_views(filename, object_types, "BPI17csv", file_type="csv", params=parameters, k=k, weight=weight, selection_method=selection_method)


def compute_views_for_bpi14(k=7, weight=0.5, selection_method="mmr"):
    filename = 'data/BPIC14.jsonocel'
    object_types = ["ConfigurationItem", "ServiceComponent", "Incident", "Interaction", "Change", "Case_R", "KM"]

    assert k <= len(object_types), "k must be less than the number of object types"
    compute_views(filename, object_types, "BPI14", k=k, weight=weight, selection_method=selection_method)


def compute_views_for_order_management(k=5, weight=0.5, selection_method="mmr"):
